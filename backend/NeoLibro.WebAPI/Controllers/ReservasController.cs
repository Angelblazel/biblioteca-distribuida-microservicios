...
namespace NeoLibroAPI.Controllers
{
    ...
    public class ReservasController : ControllerBase
    {
        ...
        public ReservasController(IReservaBusiness reservaBusiness, IPublishEndpoint publishEndpoint)
        {
        ...
        public async Task<IActionResult> Aprobar(int id)
        {
            ...
            if (result > 0)
            {
                try
                {
                    var prestamoRepository = HttpContext.RequestServices.GetRequiredService<IPrestamoRepository>();
                    var prestamo = prestamoRepository.ObtenerPorId(result);
                    if (prestamo != null)
                    {
                        var usuarioRepository = HttpContext.RequestServices.GetRequiredService<IUsuarioRepository>();
                        var usuario = usuarioRepository.ObtenerPorId(prestamo.UsuarioID);
                        var email = usuario?.EmailInstitucional ?? "";

                        var ejemplarRepository = HttpContext.RequestServices.GetRequiredService<IEjemplarRepository>();
                        var ejemplar = ejemplarRepository.ObtenerPorId(prestamo.EjemplarID);

                        var libroRepository = HttpContext.RequestServices.GetRequiredService<ILibroRepository>();
                        var libro = ejemplar != null ? libroRepository.ObtenerPorId(ejemplar.LibroID) : null;
                        var titulo = libro?.Titulo ?? "";

                        await _publishEndpoint.Publish<PrestamoCreadoEvent>(new PrestamoCreadoEvent
                        {
                            PrestamoId = prestamo.PrestamoID,
                            UsuarioId = prestamo.UsuarioID,
                            EmailUsuario = email,
                            TituloLibro = titulo,
                            FechaDevolucionPrevista = prestamo.FechaVencimiento
                        });
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"[ReservasController] Error al publicar PrestamoCreadoEvent: {ex.Message}");
                }
