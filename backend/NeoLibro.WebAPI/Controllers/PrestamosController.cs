...
namespace NeoLibroAPI.Controllers
{
    ...
    public class PrestamosController : ControllerBase
    {
        ...
        public PrestamosController(IPrestamoBusiness prestamoBusiness, IPublishEndpoint publishEndpoint)
        {
        ...
        public async Task<IActionResult> CrearPrestamo([FromBody] CrearPrestamoRequest request)
        {
            ...
            if (resultado)
            {
                ...
                {
                    var usuarioRepository = HttpContext.RequestServices.GetRequiredService<IUsuarioRepository>();
                    var usuario = usuarioRepository.ObtenerPorId(usuarioDestinoId);
                    var email = usuario?.EmailInstitucional ?? "";

                    var prestamos = _prestamoBusiness.ListarPrestamosActivosPorUsuario(usuarioDestinoId);
                    var prestamoCreado = prestamos
                        .Where(p => p.EjemplarID == request.EjemplarID)
                    ...
                    if (prestamoCreado != null)
                    {
                        ...
                        {
                            PrestamoId = prestamoCreado.PrestamoID,
                            UsuarioId = usuarioDestinoId,
                            EmailUsuario = email,
                            TituloLibro = prestamoCreado.LibroTitulo,
                            FechaDevolucionPrevista = prestamoCreado.FechaVencimiento
                        });
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"[PrestamosController] Error al publicar PrestamoCreadoEvent: {ex.Message}");
                }

                return Ok(new { mensaje = "Préstamo creado correctamente" });
            }

            return BadRequest(new { mensaje = "No se pudo crear el préstamo. Verifique que el ejemplar esté disponible." });
        }
